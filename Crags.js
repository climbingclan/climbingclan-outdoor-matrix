...
let sheet = setupSheet("Crags")
  ...
  var results = stmt.executeQuery('select distinct `cc_outdoor_location` "Venue", pd.cc_volunteer AS "Selected Roles", db.`first_name` "Name",`nickname` "FB Name",`admin-first-timer-outdoor` "New?",`climbing-discipline-preference` "Sp or Tr?",  `skills-trad-climbing` "Trad Skills", `skills-sport-climbing` "Sport Skills", gear_bringing_rack "Rack",gear_bringing_rope "Rope",`climbing-indoors-toproping-grades` "Indoor TR",`transport-need-lift` "Lift?", `transport-leaving-location` "Location",`admin-outdoors-requests-notes` "Requests and notes", climbing_outdoor_crag_location_preference "Crag pref",scores_attendance_reliability_score_cached "Attendce %", `skills-belaying` "Belaying Skills",pd.order_id "Order ID" from wp_member_db db LEFT JOIN wp_order_product_customer_lookup pd on pd.user_id = db.id JOIN wp_member_db_gear gr on pd.user_id = gr.id JOIN wp_member_db_skills sk on pd.user_id = sk.id where product_id=' + cell + ' AND status in ("wc-processing", "wc-onhold", "wc-on-hold") AND cc_attendance="pending" order by FIELD(`cc_outdoor_location`,"none","", "bamford_edge", "harpurhill",  "wilton1", "wilton2", "wilton3", "wilton4", "cadshaw", "windgather") Asc,`cc_outdoor_location`, pd.`cc_volunteer` desc, gear_bringing_rack Desc, skills_belaying_lead Desc, `skills-trad-climbing` Asc,`skills-sport-climbing` Asc, `climbing-indoors-toproping-grades` Desc');
